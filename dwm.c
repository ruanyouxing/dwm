...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (ev->x < (int)TEXTW(estextl,0))
			click = ClkExBarLeftStatus;
		else if (ev->x > selmon->ww - (int)TEXTW(estextr,0))
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww -tw, 0,tw, bh,0,stext,0,0);
	}
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			int mid = (m->ww - (int)TEXTW(m->sel->name,0)) / 2 - x;
			/* make sure name will not overlap on tags even when it is very long */
			mid = mid >= lrpad / 2 ? mid : lrpad / 2;
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]);
			drw_text(drw, x, 0, w, bh, lrpad / 2 + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0,0);
	...
	if (m == selmon) { /* extra status is only drawn on selected monitor */
		...
		etwr = TEXTW(estextr,0) - lrpad + 2; /* 2px right padding */
		drw_text(drw, m->ww - etwr, 0, etwr, bh, 0, estextr, 0,0);
		etwl = TEXTW(estextl,0);
		drw_text(drw, 0, 0, etwl, bh, 0, estextl, 0, 0);
